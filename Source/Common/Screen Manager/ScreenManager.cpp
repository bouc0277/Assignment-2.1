...
#include "../Menus/Credit.h"
#include "../Menus/Pause.h"
#include "../Menus/GameSettings.h"
#include <time.h>
#include "../../Common/Constants/Menu/MenuConstants.h"
...
void ScreenManager::changeTwoPlayers()
{
	for(unsigned int i = 0; i < m_Screens.size(); i++)
	{
		if(strcmp(m_Screens.at(i)->getName(), GAME_SETTINGS_SCREEN_NAME) == 0)
		{
			m_CurrentScreen->twoPlayersChange();
		}
	}
}

void ScreenManager::changeOnePlayers()
{
	for(unsigned int i = 0; i < m_Screens.size(); i++)
	{
		if(strcmp(m_Screens.at(i)->getName(), GAME_SETTINGS_SCREEN_NAME) == 0)
		{
			m_CurrentScreen->onePlayersChange();
		}
	}
}